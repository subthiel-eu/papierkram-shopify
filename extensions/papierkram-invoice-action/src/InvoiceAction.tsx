...
import type { Api } from "@shopify/ui-extensions/admin.order-details.action.render";
...
export default async function extension() {
  render(<InvoiceAction />, document.body);
}
...
function InvoiceAction() {
  ...
  const existing = context?.documents.filter((doc) => doc.kind === "invoice") ?? [];
  ...
  async function submit() {
    ...
    if (!created.ok || !created.document) {
      setError(created.error ?? "Die Rechnung konnte nicht angelegt werden.");
      setBusy(false);
      return;
    }
    ...
    if (delivery !== "draft") {
        ...
        ...(delivery === "email"
          ? {
              email: {
                ...
                subject: `Ihre Rechnung ${created.document.documentNo ?? ""}`.trim(),
                body:
      ...
      if (!delivered.ok) {
        // Die Rechnung existiert bereits - nur der Versand ist gescheitert.
        setError(
          `Die Rechnung wurde angelegt, der Versand schlug jedoch fehl: ${delivered.error ?? "unbekannter Fehler"}`,
        );
        setBusy(false);
        return;
      }
    ...
    setDone(
      created.document.documentNo
        ? `Rechnung ${created.document.documentNo} angelegt.`
        : "Rechnung als Entwurf angelegt.",
    );
    setBusy(false);
  }
  ...
  return (
    <s-admin-action
      heading="Rechnung in Papierkram anlegen"
      loading={busy}
    >
      ...
      <s-stack direction="block" gap="base">
        ...
        {existing.length > 0 && !done ? (
          <s-banner tone="warning" heading="Es gibt bereits eine Rechnung">
            <s-paragraph>
